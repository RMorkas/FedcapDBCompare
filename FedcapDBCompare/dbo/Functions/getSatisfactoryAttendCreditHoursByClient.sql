...
Create FUNCTION [dbo].[getSatisfactoryAttendCreditHoursByClient]
(
	@caseClientId int, @monthDate smalldatetime
)
RETURNS int
AS
BEGIN
	...
	SELECT @isTeenparent = Count(clientCalc.CaseClientId) from dbo.ClientMonthlyCalc clientCalc With(nolock) 
	WHERE clientCalc.CaseClientId = @caseClientId AND clientCalc.MonthDate = @monthDate
	AND
	...
	Select @CoreActualHours = ISNULL(ClientCoreActualHours,0), @CoreMin = CEILING(caseType.MinRequiredCoreAvgHours * 4.33)
	...
	Where CC.CaseClientId = @caseClientId AND caseCalc.MonthDate = @monthDate
