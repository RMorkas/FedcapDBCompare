CREATE TABLE [dbo].[ClientFcaActivity] (
    [ClientFcaActivityId] INT      IDENTITY (1, 1) NOT NULL,
    [ClientFcaId]         INT      NULL,
    [ActivityId]          INT      NULL,
    [ParentLastSave]      DATETIME NULL,
    CONSTRAINT [PK_ClientFcaActivity] PRIMARY KEY CLUSTERED ([ClientFcaActivityId] ASC),
    CONSTRAINT [FK_ClientFcaActivity_ClientFca] FOREIGN KEY ([ClientFcaId]) REFERENCES [dbo].[ClientFca] ([ClientFcaId])
);


GO
CREATE TRIGGER [dbo].[ClientFcaActivityLog] ON [dbo].[ClientFcaActivity] 
FOR UPDATE , INSERT
AS
...
Set @TableName = N'ClientFcaActivity'
