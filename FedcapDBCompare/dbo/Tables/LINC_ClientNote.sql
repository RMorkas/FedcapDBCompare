CREATE TABLE [dbo].[LINC_ClientNote] (
    ...
    CONSTRAINT [PK_LINC_ClientNote] PRIMARY KEY CLUSTERED ([ClientNoteId] ASC),
    CONSTRAINT [FK_LINC_ClientNote_Client] FOREIGN KEY ([ClientId]) REFERENCES [dbo].[Client] ([ClientId])
);
...
CREATE TRIGGER [dbo].[ClientNote] ON [dbo].[LINC_ClientNote] 
FOR UPDATE , INSERT
AS
...
Set @TableName = N'LINC_ClientNote'
SET @ChildPKName = 'ClientNoteId'
...
While (@index <= @count)
	...
	END
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Read From Enum', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'LINC_ClientNote', @level2type = N'COLUMN', @level2name = N'NoteTypeId';
