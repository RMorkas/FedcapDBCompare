CREATE TABLE [dbo].[CV_Education] (
    ...
    CONSTRAINT [PK_CV_Education] PRIMARY KEY CLUSTERED ([EducationId] ASC),
    CONSTRAINT [FK_CV_Education_CV_Education] FOREIGN KEY ([ResumeId]) REFERENCES [dbo].[CV_BaseResume] ([ResumeId]),
    CONSTRAINT [FK_CV_Education_Enumes] FOREIGN KEY ([DegreeTypeId]) REFERENCES [dbo].[Enumes] ([EnumId])
);


GO
CREATE TRIGGER [dbo].[educationLog] ON [dbo].[CV_Education] 
FOR UPDATE , INSERT
AS
...
Set @TableName = N'CV_Education'
SET @ChildPKName = 'EducationId'
...
While (@index <= @count)
	...
	END
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Read from Enum Table', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'CV_Education', @level2type = N'COLUMN', @level2name = N'DegreeTypeId';
