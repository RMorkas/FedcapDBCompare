CREATE TABLE [dbo].[CV_JobOrderInterviewTime] (
    [JobInterviewTimeId] INT          IDENTITY (1, 1) NOT NULL,
    [JobOrderId]         INT          NULL,
    [InterviewDay]       VARCHAR (50) NULL,
    [StartTime]          DATETIME     NULL,
    [EndTime]            DATETIME     NULL,
    CONSTRAINT [PK_CV_JobInterviewTimes] PRIMARY KEY CLUSTERED ([JobInterviewTimeId] ASC),
    CONSTRAINT [FK_CV_JobInterviewTimes_CV_JobOrder] FOREIGN KEY ([JobOrderId]) REFERENCES [dbo].[CV_JobOrder] ([JobOrderId])
);


GO
CREATE TRIGGER [dbo].[interviewLog] ON [dbo].[CV_JobOrderInterviewTime] 
FOR UPDATE , INSERT
AS
...
SET @PartentTable = 'CV_JobOrder'
Set @TableName = N'CV_JobOrderInterviewTime'
SET @ChildPKName = 'JobInterviewTimeId'
