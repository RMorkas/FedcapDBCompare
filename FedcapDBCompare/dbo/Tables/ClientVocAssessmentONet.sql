CREATE TABLE [dbo].[ClientVocAssessmentONet] (
    [ClientVocAssessmentONetId] INT            IDENTITY (1, 1) NOT NULL,
    [VocAssessmentId]           INT            NULL,
    [NETCode]                   VARCHAR (80)   NULL,
    [NETDescription]            VARCHAR (3000) NULL,
    [ParentLastSave]            DATETIME       NULL,
    CONSTRAINT [PK_ClientVocAssessmentONet] PRIMARY KEY CLUSTERED ([ClientVocAssessmentONetId] ASC),
    CONSTRAINT [FK_ClientVocAssessmentONet_ClientVocAssessment] FOREIGN KEY ([VocAssessmentId]) REFERENCES [dbo].[ClientVocAssessment] ([VocAssessmentId])
);


GO
CREATE TRIGGER [dbo].[VocONetLog] ON dbo.ClientVocAssessmentONet 
FOR UPDATE , INSERT
AS
...
SET @PartentTable = 'ClientVocAssessment'
Set @TableName = N'ClientVocAssessmentONet'
