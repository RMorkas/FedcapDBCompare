CREATE TABLE [dbo].[ClientFcaSupportService] (
    [ClientFcaSupportServiceId] INT      IDENTITY (1, 1) NOT NULL,
    [ClientFcaId]               INT      NULL,
    [SupportServiceId]          INT      NULL,
    [SupportServiceFrequencyId] INT      NULL,
    [Amount]                    INT      NULL,
    [ParentLastSave]            DATETIME NULL,
    CONSTRAINT [PK_ClientFcaSupportService] PRIMARY KEY CLUSTERED ([ClientFcaSupportServiceId] ASC),
    CONSTRAINT [FK_ClientFcaSupportService_ClientFca] FOREIGN KEY ([ClientFcaId]) REFERENCES [dbo].[ClientFca] ([ClientFcaId])
);


GO
CREATE TRIGGER [dbo].[ClientFcaSupportServiceLog] ON [dbo].[ClientFcaSupportService] 
FOR UPDATE , INSERT
AS
...
SET @PartentTable = 'ClientFca'
Set @TableName = N'ClientFcaSupportService'
