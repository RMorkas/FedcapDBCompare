CREATE TABLE [dbo].[WellnessPLan] (
    [WellnessPLanId]       INT            IDENTITY (1, 1) NOT NULL,
    [CompanyId]            INT            NULL,
    [ClientId]             INT            NULL,
    [IstakingMedications]  BIT            NULL,
    [HasTreatingPhysician] BIT            NULL,
    [HasUpcomingAppt]      BIT            NULL,
    [physicianAppt]        VARCHAR (80)   NULL,
    [ApptDate]             SMALLDATETIME  NULL,
    [ApptNotes]            VARCHAR (3000) NULL,
    [IsRiskPregnancy]      BIT            NULL,
    [RehabDate]            SMALLDATETIME  NULL,
    [RehabPeriodId]        INT            NULL,
    [WellnessPlanGoal]     VARCHAR (3000) NULL,
    [Signature]            VARCHAR (MAX)  NULL,
    [WellnessDate]         DATETIME       NULL,
    ...
    [IsLocked]             BIT            CONSTRAINT [DF_WellnessPLan_IsLocked] DEFAULT ((0)) NOT NULL,
    [IsDeleted]            BIT            CONSTRAINT [DF_WellnessPLan_IsDeleted] DEFAULT ((0)) NOT NULL,
    CONSTRAINT [PK_WellnessPLan] PRIMARY KEY CLUSTERED ([WellnessPLanId] ASC),
    CONSTRAINT [FK_WellnessPLan_Client] FOREIGN KEY ([ClientId]) REFERENCES [dbo].[Client] ([ClientId])
);


GO
CREATE TRIGGER [dbo].[WellnessPLanAudit] ON dbo.WellnessPLan 
after UPDATE , INSERT, Delete
AS
Declare @TableName varchar(50), @formId int, @updatedBy varchar(80), @isDeleted bit
Set @TableName = 'WellnessPLan'
...
BEGIN
	...
	Select @formId, WellnessPLanId, [CreatedAt] , [CreatedBy], 'Insert' From inserted
END
ELSE IF(@isDeleted = 0 or @isDeleted IS NULL)
BEGIN
	Insert into [dbo].[FormAudit] (FormId, FormRecordId, SavedDate, SavedBy, TransType)
	Select @formId, WellnessPLanId, [UpdatedAt] , [UpdatedBy] , 'Update' From inserted
END
ELSE IF(@isDeleted = 1)
BEGIN
	Insert into [dbo].[FormAudit] (FormId, FormRecordId, SavedDate, SavedBy, TransType)
	Select @formId, WellnessPLanId, [UpdatedAt] , [UpdatedBy] , 'Delete' From inserted
END

GO
CREATE TRIGGER [dbo].[WellnessPLanLog] ON dbo.WellnessPLan 
FOR UPDATE , INSERT
AS
...
----****************************
----*/
Set @ChildTableName = 'WellnessPLanDiagnosis'
SET @ChildPKName = 'WellnessPLanDiagnosisId'

Exec UpdateChildTables @ChildTableName , @ChildPKName , @PKFieldName , @PKFieldValue

Set @ChildTableName = 'WellnessPLanMedication'
SET @ChildPKName = 'WellnessPLanMedicationId'

Exec UpdateChildTables @ChildTableName , @ChildPKName , @PKFieldName , @PKFieldValue

Set @ChildTableName = 'WellnessPLanPhysician'
SET @ChildPKName = 'WellnessPLanPhysicianId'
