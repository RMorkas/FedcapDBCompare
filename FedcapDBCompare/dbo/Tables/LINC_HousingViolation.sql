CREATE TABLE [dbo].[LINC_HousingViolation] (
    [ViolationId]         INT           IDENTITY (1, 1) NOT NULL,
    [ClientId]            INT           NULL,
    [ViolationAssessedId] INT           NULL,
    [ViolationRepairedId] INT           NULL,
    [ComplaintNo]         VARCHAR (15)  NULL,
    [DateReported]        SMALLDATETIME NULL,
    [ContactPhone]        VARCHAR (50)  NULL,
    [ViolationNote]       VARCHAR (MAX) NULL,
    [CreatedBy]           VARCHAR (80)  NULL,
    [CreatedAt]           DATETIME      NULL,
    [UpdatedBy]           VARCHAR (80)  NULL,
    [UpdatedAt]           DATETIME      NULL,
    CONSTRAINT [PK_LINC_HousingViolation] PRIMARY KEY CLUSTERED ([ViolationId] ASC),
    CONSTRAINT [FK_LINC_HousingViolation_Client] FOREIGN KEY ([ClientId]) REFERENCES [dbo].[Client] ([ClientId])
);
...
CREATE TRIGGER [dbo].[HousingViolation] ON [dbo].[LINC_HousingViolation] 
FOR UPDATE , INSERT
AS
...
Set @TableName = N'LINC_HousingViolation'
SET @ChildPKName = 'ViolationId'
...
While (@index <= @count)
	...
	END
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Read from Enum', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'LINC_HousingViolation', @level2type = N'COLUMN', @level2name = N'ViolationAssessedId';


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Read from Enum', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'LINC_HousingViolation', @level2type = N'COLUMN', @level2name = N'ViolationRepairedId';
