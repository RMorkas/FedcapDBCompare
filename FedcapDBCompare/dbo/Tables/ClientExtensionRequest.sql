CREATE TABLE [dbo].[ClientExtensionRequest] (
    [ExtensionRequestId]       INT            IDENTITY (1, 1) NOT NULL,
    [CompanyId]                INT            NULL,
    [ClientId]                 INT            NULL,
    [DateFrom]                 SMALLDATETIME  NULL,
    [DateTo]                   SMALLDATETIME  NULL,
    [ReasonBenefitsExtendedId] INT            NULL,
    [AdditionalDetails]        VARCHAR (3000) NULL,
    [FedcapDecisionId]         INT            NULL,
    [DenialReason]             VARCHAR (3000) NULL,
    [IsVerificationReceived]   BIT            NULL,
    [IsReConsideration]        BIT            NULL,
    [FormVersionId]            INT            NULL,
    [CreatedBy]                VARCHAR (80)   NULL,
    [CreatedAt]                DATETIME       NULL,
    [UpdatedBy]                VARCHAR (80)   NULL,
    [UpdatedAt]                DATETIME       NULL,
    [IsLocked]                 BIT            CONSTRAINT [DF_ClientExtensionRequest_IsLocked] DEFAULT ((0)) NOT NULL,
    [RequestStatus]            INT            CONSTRAINT [DF_ClientExtensionRequest_IsApproved] DEFAULT ((0)) NOT NULL,
    [IsDeleted]                BIT            CONSTRAINT [DF_ClientExtensionRequest_IsDeleted] DEFAULT ((0)) NOT NULL,
    [ScanImageId]              INT            NULL,
    CONSTRAINT [PK_ClientExtensionRequest] PRIMARY KEY CLUSTERED ([ExtensionRequestId] ASC)
);


GO
CREATE TRIGGER [dbo].[ExtensionRequestAudit] ON dbo.ClientExtensionRequest 
after UPDATE , INSERT, Delete
AS
Declare @TableName varchar(50), @formId int, @updatedBy varchar(80), @isDeleted bit
Set @TableName = 'ClientExtensionRequest'
...
BEGIN
	...
	Select @formId, ExtensionRequestId, [CreatedAt] , [CreatedBy], 'Insert' From inserted
END
ELSE IF(@isDeleted = 0 or @isDeleted IS NULL)
BEGIN
	Insert into [dbo].[FormAudit] (FormId, FormRecordId, SavedDate, SavedBy, TransType)
	Select @formId, ExtensionRequestId, [UpdatedAt] , [UpdatedBy] , 'Update' From inserted
END
ELSE IF(@isDeleted = 1)
BEGIN
	Insert into [dbo].[FormAudit] (FormId, FormRecordId, SavedDate, SavedBy, TransType)
	Select @formId, ExtensionRequestId, [UpdatedAt] , [UpdatedBy] , 'Delete' From inserted
END

GO
CREATE TRIGGER [dbo].[ExtensionRequestLog] ON dbo.ClientExtensionRequest 
FOR UPDATE , INSERT
AS
...
SET @RetryCount = 0
SET @MaxRetry = 3
Set @TableName = N'ClientExtensionRequest'
...
2: Request Approved
3: Request Denied', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'ClientExtensionRequest', @level2type = N'COLUMN', @level2name = N'RequestStatus';
