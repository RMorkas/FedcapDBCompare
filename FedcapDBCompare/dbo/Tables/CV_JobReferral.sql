CREATE TABLE [dbo].[CV_JobReferral] (
    [JobReferralId]           INT           IDENTITY (1, 1) NOT NULL,
    [CompanyId]               INT           NULL,
    [IsActive]                BIT           NULL,
    [ResumeId]                INT           NULL,
    [ClientId]                INT           NULL,
    [JobOrderId]              INT           NULL,
    [EmpLocatContactId]       INT           NULL,
    [SiteId]                  INT           NULL,
    [InterviewDate]           DATETIME      NULL,
    [InterviewTime]           DATETIME      NULL,
    [InteriewAddress]         VARCHAR (100) NULL,
    [City]                    VARCHAR (50)  NULL,
    [State]                   VARCHAR (50)  NULL,
    [ZipCode]                 VARCHAR (20)  NULL,
    [IsSameAsEmployerAddress] BIT           NULL,
    [TransitType]             VARCHAR (50)  NULL,
    [TravelingFromId]         INT           NULL,
    [FromStreetAddress]       VARCHAR (50)  NULL,
    [FromCity]                VARCHAR (25)  NULL,
    [FromState]               VARCHAR (25)  NULL,
    [FromZipCode]             VARCHAR (20)  NULL,
    [IsDeleted]               BIT           NULL,
    [CreatedBy]               VARCHAR (80)  NULL,
    [CreatedAt]               DATETIME      NULL,
    [UpdatedBy]               VARCHAR (80)  NULL,
    [UpdatedAt]               DATETIME      NULL,
    CONSTRAINT [PK_CV_JobReferrals] PRIMARY KEY CLUSTERED ([JobReferralId] ASC),
    CONSTRAINT [FK_CV_JobReferrals_CV_JobOrder] FOREIGN KEY ([JobOrderId]) REFERENCES [dbo].[CV_JobOrder] ([JobOrderId]),
    CONSTRAINT [FK_CV_JobReferrals_CV_Resume] FOREIGN KEY ([ResumeId]) REFERENCES [dbo].[CV_BaseResume] ([ResumeId]),
    CONSTRAINT [FK_CV_JobReferrals_EmployerLocationContacts] FOREIGN KEY ([EmpLocatContactId]) REFERENCES [dbo].[EmployerLocationContacts] ([EmpLocatContactId])
);


GO
CREATE TRIGGER [dbo].[referralLog] ON [dbo].[CV_JobReferral] 
For UPDATE , INSERT
AS
...
SET @RetryCount = 0
SET @MaxRetry = 3
Set @TableName = N'CV_JobReferral'
...
While (@index <= @count)
	...
	END
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Enum Table', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'CV_JobReferral', @level2type = N'COLUMN', @level2name = N'TravelingFromId';
