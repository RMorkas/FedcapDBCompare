CREATE procedure [dbo].[GenerateACFReport_01_2017] (@startdate datetime, @enddate datetime)
as 
begin
...
where dbo.client.clientid in (
 select distinct orientation.ClientId  From dbo.Schedule orientation where orientation.ClassInstRoomId =11 and 
   (datediff(d,'01/30/2017', orientation.[date]) >= 0 and datediff(d,orientation.[date],'01/31/2017') >= 0 ) and orientation.IsDeleted = 0 and orientation.IsVisible =1)
