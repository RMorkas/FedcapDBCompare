...
CREATE proc [dbo].[Rpt_PlacementInfoCRS]
@companyId int,
@DBName nvarchar(max),
...
AS
Declare @SqlString NVARCHAR(Max), @filter NVARCHAR(Max)
...
'						 Else ''''''''
						 ...
						 AssistedUsers.FirstName + '' '' + AssistedUsers.LastName AS AssistedBy, HRACases_1.IsPrivilegeRequired, EnumNonBillableReason.Item AS NonBillableReason,
						 dbo.[CaseType].TypeName
FROM            dbo.PlacementEntry with(nolock) INNER JOIN
                         [dbo].[VW_HRACases] AS HRACases_1 WITH(NOLOCK) ON dbo.PlacementEntry.HRACaseID = HRACases_1.HRACaseID AND dbo.PlacementEntry.CompanyId = HRACases_1.CompanyId INNER JOIN
                         ...
                         dbo.[User] AS AssistedUsers ON dbo.PlacementEntry.AssistedById = AssistedUsers.UserID LEFT OUTER JOIN
				    dbo.[Case] with(nolock) ON HRACases_1.ActiveCaseId = dbo.[Case].[CaseId] LEFT OUTER JOIN  
				    dbo.[CaseType] with(nolock) ON dbo.[Case].[CaseTypeId] = dbo.[CaseType].[CaseTypeId] LEFT OUTER JOIN
