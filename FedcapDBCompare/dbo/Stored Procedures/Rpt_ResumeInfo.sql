...
CREATE proc [dbo].[Rpt_ResumeInfo]
@companyId int,
@DBName nvarchar(max),
...
SELECT   distinct  dbo.CV_BaseResume.ResumeId, dbo.CV_BaseResume.HrCaseId, dbo.CV_BaseResume.FirstName, dbo.CV_BaseResume.LastName, HRACases_1.HRACaseNumber, 
                         HRACases_1.Suffix, HRACases_1.LineNumber, HRACases_1.SSN, HRACases_1.CIN,dbo.CV_BaseResume.PhoneNumber, dbo.CV_BaseResume.HomePhone,
						...
						CASE
							WHEN '  + '' + Cast(@companyId as nvarchar(max)) + '' + ' = ''1'' THEN 
									NYCWAY_RegionI.fed.Schedule_FindLocationOfAppointment(HRACases_1.SSN,dbo.CV_BaseResume.HrCaseId,1000,'+ '''' + CONVERT(nvarchar(max),GETDATE(),101) + '''' +')
							WHEN '  + '' + Cast(@companyId as nvarchar(max)) + '' + ' = ''2'' THEN 
									NYCWAY_History.fed.Schedule_FindLocationOfAppointment(HRACases_1.SSN,dbo.CV_BaseResume.HrCaseId,1000,' + '''' + CONVERT(nvarchar(max),GETDATE(),101) + '''' + ')
							ELSE NULL
						END AS SiteName, ISNULL(HRACases_1.IsPrivilegeRequired,0) AS IsPrivilegeRequired

						 
FROM            dbo.CV_BaseResume with(nolock) INNER JOIN
                dbo.VW_HRACases AS HRACases_1 with(nolock) ON dbo.CV_BaseResume.HrCaseId = HRACases_1.HRACaseID 
				AND dbo.CV_BaseResume.CompanyId =  HRACases_1.CompanyId	Left Outer Join
                dbo.HRAActionCodes with(nolock) ON (HRACases_1.SSN = dbo.HRAActionCodes.ssn AND Replace(dbo.HRAActionCodes.ssn,'' '' ,'''') <>  '''' 
				AND dbo.HRAActionCodes.[action code] = ''169E'' AND Replace(dbo.HRAActionCodes.[completion code], '' '', '''' ) = '''' )
				--OR 
				--((dbo.HRAActionCodes.ssn IS NULL or Replace(dbo.HRAActionCodes.ssn,'' '' ,'''') =  '''') AND HRACases_1.CIN = dbo.HRAActionCodes.cin)	
				LEFT OUTER JOIN
                dbo.ClientDetail with(nolock) ON HRACases_1.HRACaseID = dbo.ClientDetail.ClientId AND HRACases_1.CompanyId = dbo.ClientDetail.CompanyId LEFT OUTER JOIN
...
+
' AND ( ' + '' + Cast(@isActivePostIPE as nvarchar(max)) + '' + ' = ''0''  OR ' + ' dbo.HRAActionCodes.[action code] IS NOT NULL )'
+
