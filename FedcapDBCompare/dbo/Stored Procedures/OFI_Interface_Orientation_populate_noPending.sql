Create  procedure [dbo].[OFI_Interface_Orientation_populate_noPending] (@statusmessage varchar(max) output)
as
begin
...
SELECT      distinct  dbo.client.ReferringOfficeNumber as [OFI office], client.ClientId, client.clientno, dbo.[case].caseno,  
...
and DATEDIFF(d,missed_orientation.[Date], future_orientation.[Date]) > 0 and DATEDIFF(d,getdate(), future_orientation.[Date]) > 0
and Future_orientation.attendancestatus  = 0 
...
--not attended
union
SELECT      distinct dbo.client.ReferringOfficeNumber as [OFI office],client.ClientId, dbo.client.clientno, dbo.[case].caseno, 
...
FROM  dbo.Schedule missed_orientation with (nolock) inner join dbo.client with (nolock) on dbo.client.clientid =missed_orientation.clientid 
 and clientno not like '%test%' and missed_orientation.isvisible = 1 and missed_orientation.IsDeleted <> 1 
inner join dbo.[case] on  dbo.[case].caseid =dbo.client.activecaseid --and dbo.[case].StatusId = 'P'
...
FROM  dbo.Schedule missed_orientation with (nolocK) inner join dbo.client with (nolocK) on dbo.client.clientid =missed_orientation.clientid 
 ...
 and not exists ( select attended_orientation.scheduleid from dbo.Schedule attended_orientation with (nolocK) where attended_orientation.clientid = missed_orientation.clientid and 
attended_orientation.ClassInstRoomId = @roomnumber AND attended_orientation.attendancestatus in (1,2,3,4) and isnull(attended_orientation.EventId,0) <> 74 and  
attended_orientation.IsDeleted <> 1
and  datediff(d,missed_orientation.date,attended_orientation.date) > 0 )  

inner join dbo.[case] with (nolocK) on  dbo.[case].caseid =dbo.client.activecaseid and dbo.[case].StatusId = 'P'
...
and missed_orientation.ScheduleId <> future_orientation.ScheduleId and  future_orientation.IsDeleted <> 1 ) and future_orientation.attendancestatus = 0 )) 
