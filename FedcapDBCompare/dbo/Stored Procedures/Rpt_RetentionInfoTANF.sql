...
CREATE proc [dbo].[Rpt_RetentionInfoTANF]
@companyId int,
@DBName varchar(max),
...
Declare @SqlString varchar(Max) , @sqlCMDataTable varchar(Max) , @sqlLoginTable Nvarchar(Max)
...
FROM            dbo.PlacementEntry with(nolock) INNER JOIN
                         ...
                         dbo.Enumes AS Enumes_2 ON dbo.PlacementEntry.PlacementSourceId = Enumes_2.EnumId Left OUTER JOIN
                         ...
                         dbo.[User] AS LeadUsers ON dbo.PlacementEntry.PlacementLeadUserId = LeadUsers.UserID
WHERE
