...
CREATE proc [dbo].[Rpt_ResumeInfo_BTW]
@companyId int,
@DBName nvarchar(max),
...
FROM            dbo.CV_BaseResume with(nolock) INNER JOIN
				...
				Outer Apply
				(
					Select Distinct * FROM [ALLSECTOR_BTW].[fedcapbtw].[dbo].[Timekeeping_Import] as ActiveClient 
					Where
					ActiveClient.HRACaseId = HRACases_1.HRACaseID
...
+
' AND ( ' + '' + Cast(@isActivePostIPE as nvarchar(max)) + '' + ' = ''0''  OR ' + ' (Replace(ActiveClient.TerminationDate, '' '' ,'''')  = '''' 
					OR
					(dateadd(day,0,datediff(day,0, ActiveClient.TerminationDate))>= dateadd(day,0,datediff(day,0, GETDATE()))) ) )'
+
