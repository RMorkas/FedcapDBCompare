...
CREATE proc [dbo].[Rpt_ResumeInfo_CRS]
@companyId int,
@DBName nvarchar(max),
...
Set @SqlString = N'
SELECT   distinct  dbo.CV_BaseResume.ResumeId, dbo.CV_BaseResume.HrCaseId, dbo.CV_BaseResume.FirstName, dbo.CV_BaseResume.LastName, HRACases_1.HRACaseNumber AS HRACaseNumber, 
...
FROM            dbo.CV_BaseResume with(nolock) INNER JOIN
                VW_HRACases AS HRACases_1 with(nolock) ON dbo.CV_BaseResume.HrCaseId = HRACases_1.HRACaseID
                ...
                dbo.CV_Position with(nolock) ON dbo.CV_BaseResume.ResumeId = dbo.CV_Position.ResumeId inner Join
				dbo.Client client WIth(NOLOCK) on HRACases_1.HRACaseID = client.ClientId AND HRACases_1.CompanyId =  client.CompanyId LEFT OUTER JOIN
				[dbo].[Site] as clientSite WITH(nolock) on client.SiteId = clientSite.SiteId
				Outer Apply
						(
