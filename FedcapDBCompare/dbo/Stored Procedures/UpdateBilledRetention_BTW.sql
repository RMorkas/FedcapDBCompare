...
Create Proc [dbo].[UpdateBilledRetention_BTW]
AS
BEGIN
...
SELECT [ID]
  ...
  FROM [ALLSECTOR_BTW].[FEDCAPBTW].[dbo].[cmconvertlist]
  where DocType in (8726,8729,8731) and Status in (3,15,16)-- AND CaseId in (33, 65360,62373,65360,48 , 39081)
...
Select PlacementPeriodId,PlacementPeriod.PlacementEntryID,PeriodId,AvailabilityDate,ExpirationDate,
		Case
			When PeriodId = @period_30 then 8726
			When PeriodId = @period_90 then 8729
			When PeriodId = @period_180 then 8731
			ElSE 0
...
Where CompanyId = @compnayId AND PlacementRetentionId IS NULL) AS period Join @convertList as doc on
