...
CREATE proc [dbo].[Rpt_RetentionInfo]
@companyId int,
@DBName varchar(max),
...
Declare @SqlString varchar(Max) , @sqlCMDataTable varchar(Max) , @sqlLoginTable Nvarchar(Max), @serverName Nvarchar(max)
...
SELECT temp.*, 
	CASE 
			WHEN temp.CompanyId = 7 Then --B2W
...
FROM            dbo.PlacementEntry with(nolock) INNER JOIN
                         ...
                         @LoginTable AS LeadUsers ON dbo.PlacementEntry.PlacementLeadUserId = LeadUsers.UserID LEFT OUTER JOIN
...
execute (@sqlLoginTable + ' ' + @SqlString)